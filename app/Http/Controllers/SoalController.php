...
{
    /**
     * Create a new SoalController instance.
     *
     * @return void
     */
    ...
    public function no7()
    {
        ...
        $billdetailsCollection = collect($soal->data->response->billdetails);
        $filteredBilldetailsCollection = $billdetailsCollection->filter(function ($bill) {
            $denom = explode(':', collect($bill->body)->first());
            return trim($denom[1]) >= 100000;
        })->values();

        return $this->successResponse($filteredBilldetailsCollection);
    }
}
